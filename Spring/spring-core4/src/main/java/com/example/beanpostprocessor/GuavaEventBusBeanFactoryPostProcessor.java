package com.example.beanpostprocessor;

import com.google.common.eventbus.EventBus;

import java.util.Iterator;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.aop.framework.Advised;
import org.springframework.aop.support.AopUtils;
import org.springframework.beans.BeansException;
import org.springframework.beans.FatalBeanException;
import org.springframework.beans.factory.config.BeanFactoryPostProcessor;
import org.springframework.beans.factory.config.ConfigurableListableBeanFactory;
import org.springframework.core.annotation.AnnotationUtils;
import org.springframework.expression.Expression;
import org.springframework.expression.ExpressionException;
import org.springframework.expression.spel.standard.SpelExpressionParser;

// Suppress all warnings
@SuppressWarnings("ALL") public class GuavaEventBusBeanFactoryPostProcessor
        implements BeanFactoryPostProcessor {
    // Logger for this class
    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    // Parser for SpEL expressions
    private final SpelExpressionParser expressionParser = new SpelExpressionParser();

    // Method to post process the bean factory after its standard initialization
    @Override public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
            throws BeansException {
        // Iterate over all bean names in the factory
        for (Iterator<String> names = beanFactory.getBeanNamesIterator(); names.hasNext(); ) {
            // Get the target object of the current bean
            Object proxy = this.getTargetObject(beanFactory.getBean(names.next()));
            // Get the Subscriber annotation of the current bean's class
            final Subscriber annotation = AnnotationUtils.getAnnotation(proxy.getClass(),
                                                                        Subscriber.class);
            // If there is no Subscriber annotation, skip this bean
            if (annotation == null)
                continue;
            // Log that we are processing this bean
            this.logger.info("{}: processing bean of type {} during initialization",
                             this.getClass().getSimpleName(), proxy.getClass().getName());
            // Get the value of the Subscriber annotation
            final String annotationValue = annotation.value();
            try {
                // Parse and evaluate the annotation value as a SpEL expression
                final Expression expression = this.expressionParser.parseExpression(
                        annotationValue);
                final Object value = expression.getValue();
                // If the evaluated value is not an EventBus, log an error and return
                if (!(value instanceof EventBus)) {
                    this.logger.error(
                            "{}: expression {} did not evaluate to an instance of EventBus for " +
                            "bean of type {}", this.getClass().getSimpleName(), annotationValue,
                            proxy.getClass().getSimpleName());
                    return;
                }
                // Register the current bean with the evaluated EventBus
                final EventBus eventBus = (EventBus) value;
                eventBus.register(proxy);
            } catch (ExpressionException ex) {
                // Log an error if we were unable to parse/evaluate the expression
                this.logger.error("{}: unable to parse/evaluate expression {} for bean of type {}",
                                  this.getClass().getSimpleName(), annotationValue,
                                  proxy.getClass().getName());
            }
        }
    }

    // Method to get the target object of a JDK dynamic proxy, or return the object itself if it
    // is not a proxy
    private Object getTargetObject(Object proxy) throws BeansException {
        if (AopUtils.isJdkDynamicProxy(proxy)) {
            try {
                return ((Advised) proxy).getTargetSource().getTarget();
            } catch (Exception e) {
                throw new FatalBeanException("Error getting target of JDK proxy", e);
            }
        }
        return proxy;
    }
}
