...
public class PersistenceConfig {
    // Autowire the Environment object, which represents the environment in which the current application is running
    @Autowired
    ...
    public DataSource restDataSource() {
        ...
        dataSource.setUrl(Objects.requireNonNull(Objects.requireNonNull(env.getProperty("jdbc.url"))));
    ...
    // Define a bean for the transaction manager, which is used to manage transactions in a Hibernate environment
    @Bean
    ...
    // Define a bean for the exception translation post processor, which adds an advisor to any bean annotated with Repository so that any platform-specific exceptions are caught and then rethrown as one of Spring's unchecked data access exceptions (i.e., a subclass of DataAccessException)
    @Bean
