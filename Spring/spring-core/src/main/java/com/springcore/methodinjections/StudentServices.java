...
// The string "studentService" is the specific name of the bean to be created, instead of the
// default one.
@Component("studentService")
// This is an abstract class named StudentServices. It can't be instantiated, but it can be
// subclassed.
public abstract class StudentServices {
    ...
    // The @Lookup annotation is used on an abstract method to override and dynamically return a
    // prototype bean from the Spring Container.
    // Here it's used to get a new instance of SchoolNotification every time it's needed.
    @Lookup protected abstract SchoolNotification getNotification(String name);

    // This method appends a mark to a student's SchoolNotification. If the student doesn't have
    // a SchoolNotification yet,
    // it creates one using the getNotification method.
    public String appendMark(String name, Integer mark) {
        // The computeIfAbsent method checks if there's already a SchoolNotification for the
        // student.
        // If not, it creates one using the provided function (getNotification).
        SchoolNotification notification = notes.computeIfAbsent(name,
                                                                exists -> getNotification(name));
