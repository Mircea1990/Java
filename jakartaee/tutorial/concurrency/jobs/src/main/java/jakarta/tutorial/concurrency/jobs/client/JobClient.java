 ...
 */
@Named @RequestScoped public class JobClient implements Serializable {
    ...
    public String submit() {
        ...
        final Response response = client.target(serviceEndpoint).queryParam("jobID", getJobID())
                                        .request().header("X-REST-API-Key", token).post(null);

        FacesMessage message;
        message = (response.getStatus() == 200) ? new FacesMessage(FacesMessage.SEVERITY_INFO,
                                                                   String.format(
                                                                           "Job %d successfully " +
                                                                           "submitted",
                                                                           getJobID()), null)
