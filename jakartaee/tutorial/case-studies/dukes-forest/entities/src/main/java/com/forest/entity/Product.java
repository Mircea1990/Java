...
@Entity @Table(name = "PRODUCT") @NamedQueries(
        {@NamedQuery(name = "Product.findAll", query = "SELECT p FROM Product p"),
                ...
                @NamedQuery(name = "Product.findByImg",
                            query = "SELECT p FROM Product p WHERE p.img = :img")})
public class Product implements Serializable {
    ...
    @Basic(optional = false) @Size(min = 3, max = 45, message = "{product.name}")
    @Column(name = "NAME", nullable = false, length = 45) private String name;
    @Basic(optional = false) @DecimalMax(value = "9999.99", message = "{product.price.max}")
    @Column(name = "PRICE", nullable = false) private double price;
    @Basic(optional = false) @Size(min = 3, max = 145, message = "{product.description}")
    @Column(name = "DESCRIPTION", nullable = false, length = 45) private String description;
    @Size(min = 3, max = 45, message = "{product.img}") @Column(name = "IMG", length = 45)
    private String img;
    @Lob @Basic(fetch = FetchType.LAZY) //
    @Column(name = "IMG_SRC") @XmlTransient private byte[] imgSrc;
