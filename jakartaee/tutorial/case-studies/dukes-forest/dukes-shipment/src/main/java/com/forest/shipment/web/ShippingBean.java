...
@Named @RequestScoped public class ShippingBean implements Serializable {
    ...
    public void updateOrderStatus(final String messageID, final Status status) {
        ...
        Response response = client.target(SERVICE_ENDPOINT).path("/" + order.getId()).request(
                MEDIA_TYPE).put(Entity.text(String.valueOf(status.getStatus())));
