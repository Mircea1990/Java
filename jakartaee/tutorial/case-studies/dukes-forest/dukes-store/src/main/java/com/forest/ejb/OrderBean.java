...
/**
 * OrderBean is an EJB exposed as RESTful service Provides methods to manipulate order status and
 * query orders based on specific status.
 */
@Stateless @Path("/orders") public class OrderBean extends AbstractFacade<CustomerOrder>
    ...
    CustomerOrder order;
    @EJB OrderStatusBean statusBean;
    ...
    @PUT @Path("{orderId}") @Produces({"application/xml", "application/json"})
    ...
    public enum Status {
        ...
        PENDING_PAYMENT(2), READY_TO_SHIP(3), SHIPPED(4), CANCELLED_PAYMENT(5), CANCELLED_MANUAL(6);
        private int status;
