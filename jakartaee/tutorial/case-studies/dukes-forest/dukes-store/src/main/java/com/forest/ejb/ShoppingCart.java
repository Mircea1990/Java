...
@Named(value = "shoppingCart") @ConversationScoped public class ShoppingCart
    ...
    @EJB OrderBean facade;
    @Inject @LoggedIn Person user;
    @EJB EventDispatcherBean eventDispatcher;
    ...
    public PageNavigation checkout() {
        ...
        if (user == null) {
            JsfUtil.addErrorMessage(
                    JsfUtil.getStringFromBundle("bundles.Bundle", "LoginBeforeCheckout"));
            ...
            for (Groups g : user.getGroupsList()) {
                if (g.getName().equals("ADMINS")) {
                    ...
                    JsfUtil.addErrorMessage(JsfUtil.getStringFromBundle("bundles.Bundle",
            ...
            LOGGER.log(Level.FINEST, "{0} Sending event from ShoppingCart",
                       Thread.currentThread().getName());
            eventDispatcher.publish(event);

            JsfUtil.addSuccessMessage(
                    JsfUtil.getStringFromBundle("bundles.Bundle", "Cart_Checkout_Success"));
            clear();
        }
