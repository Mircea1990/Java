...
/**
 * CDI event handler that calls Payment service for new orders. It will intercept (observe) an
 * <code>OrderEvent</code> with
 * <code>@New</code> <b>qualifier</b>.
 *
 * @see com.forest.events.OrderEvent
 */
@Stateless public class PaymentHandler implements IOrderHandler, Serializable {
     ...
     */
    @EJB OrderBean orderBean;

    @Override @Asynchronous public void onNewOrder(@Observes @New OrderEvent event) {
        ...
        if (processPayment(event)) {
            ...
            orderBean.setOrderStatus(event.getOrderID(), String.valueOf(
    ...
    private boolean processPayment(OrderEvent order) {
        ...
        Response resp = client.target(ENDPOINT).request(MediaType.APPLICATION_XML).post(
                Entity.entity(order, MediaType.APPLICATION_XML), Response.class);
