...
@NamedQueries({@NamedQuery(name = "Student.findAllStudents",
                           query = "SELECT s FROM Student s " + "ORDER BY s.lastName"),
        @NamedQuery(name = "Student.findStudentsByStatus",
                    query = "SELECT s FROM Student s " + "WHERE s.status = :status " +
                            "ORDER BY s.lastName")}) @Entity @XmlRootElement(name = "Student")
@XmlAccessorType(XmlAccessType.FIELD) public class Student extends Person implements Serializable {
    private static final long serialVersionUID = -6358383105437539027L;
    @ManyToMany(mappedBy = "students") protected List<TutoringSession> sessions;
    protected String school;
    @Digits(integer = 2, fraction = 0, message = "{invalid.grade.level}") protected int grade;
    @Enumerated(EnumType.STRING) protected StatusType status = StatusType.OUT;
    @OneToMany(mappedBy = "student", cascade = CascadeType.ALL) @XmlTransient
    protected List<StatusEntry> statusEntries;
    protected boolean active;
    @ManyToMany @XmlTransient private List<Guardian> guardians;
