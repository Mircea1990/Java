...
@Stateless @Named @Path("/status") public class RequestBean {

    private static final Logger logger = Logger.getLogger("dukestutoring.ejb.RequestBean");
    @Resource SessionContext ctx;
    @Inject Event<StatusEvent> statusEvent;
    ...
    public String checkOut(Student student) {
        ...
        statusEvent.fire(new StatusEvent(student));
        logger.log(Level.INFO, "Student {0} is {1}",
    ...
    public void createTutoringSession() {
        try {
            ...
            logger.log(Level.INFO, "Creating new tutoring session for {0}.",
    ...
    public TutoringSession getTodaysSession() {
        ...
        SimpleDateFormat formatter = new SimpleDateFormat("E, MMM d, yyyy");
        logger.log(Level.INFO, "Finding tutoring session for {0}",
    ...
    @GET @Path("/guardian/{guardianEmail}") @Produces({"application/xml", "application/json"})
    ...
    @GET @Path("email/{guardianEmail}") @Produces({"application/xml", "application/json"})
    @RolesAllowed({"Guardian", "Administrator"})
    public List<Student> getStatusByGuardianEmail(@PathParam("guardianEmail") String email) {
        ...
        logger.log(Level.INFO, "Guardian {0}has {1} students.",
                   new Object[]{email, results.size()});
        return results;
    }

    @GET @Path("id/{guardianId}") @Produces({"application/xml", "application/json"})
    ...
    private List<Student> getStudentsByStatus(StatusType statusType) {
        ...
        Root<Student> student = cq.from(Student.class);
        cq.select(student);
        cq.where(cb.equal(student.get(Student_.status), statusType));
        cq.distinct(true);
        TypedQuery<Student> q = em.createQuery(cq);
        return q.getResultList();
    }
