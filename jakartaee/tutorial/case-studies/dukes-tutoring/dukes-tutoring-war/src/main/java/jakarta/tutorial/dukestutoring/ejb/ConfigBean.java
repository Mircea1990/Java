...
@Singleton @Startup public class ConfigBean {
    ...
    @EJB private AdminBean adminBean;
    @EJB private RequestBean requestBean;

    /**
     * Creates a new tutoring session every day at midnight.
     */
    @Schedule(dayOfWeek = "*") public void createTutoringSession() {
        logger.info("Creating today's session in ConfigBean");
        // Create a new tutoring session
        requestBean.getTodaysSession();
    }

    @Schedule(dayOfWeek = "Sun-Thu", hour = "20") public void cleanUpTutoringSession() {
