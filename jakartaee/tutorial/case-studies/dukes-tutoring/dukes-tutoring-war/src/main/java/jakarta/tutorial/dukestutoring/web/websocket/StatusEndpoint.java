...
@ServerEndpoint("/status") @Dependent public class StatusEndpoint {
        ...
        for (Session s : sessions) {
            if (s.isOpen()) {
                try {
                    ...
                    s.getBasicRemote().sendText(studentUpdate);
                    log.log(Level.INFO, "[StatusEndpoint] {0} is now {1}",
                            new Object[]{student.getName(), student.getStatus()});
                    /* Send update */
                } catch (IOException e) {
                    log.log(Level.INFO, "[StatusEndpoint] {0}", e.getMessage());
                }
    ...
    @OnOpen public void open(final Session session) {
    ...
    @OnClose public void close(final Session session) {
