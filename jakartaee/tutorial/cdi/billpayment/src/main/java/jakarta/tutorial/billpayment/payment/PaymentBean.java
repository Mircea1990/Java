...
/**
 * Bean that fires DEBIT and CREDIT payment events based on UI selection. Check server log output
 * for event handling output.
 */
@Named @SessionScoped public class PaymentBean implements Serializable {
    ...
    @Inject @Credit Event<PaymentEvent> creditEvent;
    @Inject @Debit Event<PaymentEvent> debitEvent;
    private int paymentOption = DEBIT;

    @Digits(integer = 10, fraction = 2, message = "Invalid value") private BigDecimal value;
