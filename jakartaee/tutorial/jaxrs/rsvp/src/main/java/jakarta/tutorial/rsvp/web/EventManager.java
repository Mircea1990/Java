...
@Named @SessionScoped public class EventManager implements Serializable {
    ...
    public List<Response> retrieveEventResponses() {
        ...
        try {
            Event event = client.target(baseUri).path(this.currentEvent.getId().toString()).request(
                    MediaType.APPLICATION_XML).get(Event.class);
            if (event == null) {
