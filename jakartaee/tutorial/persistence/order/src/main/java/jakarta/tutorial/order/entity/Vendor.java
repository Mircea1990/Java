...
@Entity @Table(name = "PERSISTENCE_ORDER_VENDOR") @NamedQueries(
        {@NamedQuery(name = "findVendorsByPartialName",
                ...
                @NamedQuery(name = "findVendorByCustomerOrder",
                            query = "SELECT DISTINCT l.vendorPart.vendor " +
                                    ...
                                    "WHERE co.orderId = :id " +
                                    "ORDER BY l.vendorPart.vendor.name")}) public class Vendor
    ...
    @Column(name = "VENDORNAME") public String getName() {
        return name;
    }
