...
@Entity @Table(name = "PERSISTENCE_ORDER_VENDOR_PART",
               uniqueConstraints = @UniqueConstraint(columnNames = {"PARTNUMBER", "PARTREVISION"}))
@NamedQueries({@NamedQuery(name = "findAverageVendorPartPrice",
...
public class VendorPart implements java.io.Serializable {
    ...
    @TableGenerator(name = "vendorPartGen", table = "PERSISTENCE_ORDER_SEQUENCE_GENERATOR",
                    ...
                    pkColumnValue = "VENDOR_PART_ID", allocationSize = 10) @Id
    ...
    @OneToOne @JoinColumns({@JoinColumn(name = "PARTNUMBER", referencedColumnName = "PARTNUMBER"),
    ...
    @JoinColumn(name = "VENDORID") @ManyToOne public Vendor getVendor() {
        return vendor;
    }
