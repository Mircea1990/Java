...
@Dependent @Named("MobileBatchlet") public class MobileBatchlet implements Batchlet {
    ...
    @Override public String process() throws Exception {
        ...
        /* Write the result */
        try (BufferedWriter bwriter = new BufferedWriter(new FileWriter(fileOutName, false))) {
            double percent = 100.0 * (1.0 * pageVisits) / (1.0 * totalVisits);
            bwriter.write(String.format("%d, %d, %.02f", pageVisits, totalVisits, percent));
        }
        return "COMPLETED";
    }
