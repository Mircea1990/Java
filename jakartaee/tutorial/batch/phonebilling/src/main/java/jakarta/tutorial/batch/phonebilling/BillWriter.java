 ...
 */
@Dependent @Named("BillWriter") public class BillWriter implements ItemWriter {
    ...
    @Override public void writeItems(List<Object> list) throws Exception {
        for (Object billObject : list) {
            ...
            try (BufferedWriter bwriter = new BufferedWriter(fwriter)) {
                ...
                for (CallRecord call : bill.getCalls()) {
                    ...
                    bwriter.write(
                            String.format("%s\t%s\t%s\t%02d:%02d\t%.2f", date, call.getFromNumber(),
                                          call.getToNumber(), call.getMinutes(), call.getSeconds(),
                                          call.getPrice()));
                    bwriter.newLine();
                }
                bwriter.write(" ");
                ...
                bwriter.newLine();
                bwriter.write(
                        String.format("Tax rate\t%.2f%%", bill.getTaxRate().doubleValue() * 100));
                bwriter.newLine();
