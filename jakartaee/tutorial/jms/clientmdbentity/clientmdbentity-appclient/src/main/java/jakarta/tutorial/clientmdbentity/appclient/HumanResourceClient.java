...
/**
 * The HumanResourceClient class is the client program for this J2EE application. It publishes a
 * message describing a new hire business event that other departments can act upon. It also listens
 * for a message reporting the completion of the other departments' actions and displays the
 * results.
 */
@JMSDestinationDefinition(name = "java:app/jms/HRTopic", interfaceName = "jakarta.jms.Topic",
                          destinationName = "PhysicalHRTopic") public class HumanResourceClient {

    static final Logger logger = Logger.getLogger("HumanResourceClient");
    static final Object waitUntilDone = new Object();
    static SortedSet<Object> outstandingRequests = Collections.synchronizedSortedSet(
            new TreeSet<>());
    @Resource(lookup = "java:comp/DefaultJMSConnectionFactory") private static ConnectionFactory
            connectionFactory;
    @Resource(lookup = "java:app/jms/HRTopic") private static Topic pubTopic;
    ...
    public static void main(String[] args) {
        ...
        try (JMSContext context = connectionFactory.createContext();) {
            ...
            String[] names =
                    {"Fred Verdon", "Robert Rogers", "Tom Stuart", "Mark Wilson", "James Anderson",
                            "Wayne Reynolds", "Steve Waters", "Alfred Merman", "Joe Lawrence",
                            "Jack Drake", "Harry Preston", "Bill Tudor", "Gertrude Windsor",
            ...
            for (int i = 0; i < 5; i++) {
                ...
                System.out.println(
                        ...
                        ", name " + message.getString("Name") + ", position " +
            ...
            logger.log(Level.SEVERE, "HumanResourceClient: Exception: {0}", e.toString());
        }
        System.exit(0);
    }

    /**
     * Rather than risk names being repeated, generate an array with all possible name positions in
     * a random order.
     *
    ...
    /**
     * The HRListener class implements the MessageListener interface by defining an onMessage
     * method.
     */
    static class HRListener implements MessageListener {

        /**
         * Displays the contents of a MapMessage describing the results of processing the new
         * employee, then removes the employee ID from the list of outstanding requests.
         *
        ...
        @Override public void onMessage(Message message) {
            ...
            try {
                ...
                System.out.println("  Equipment: " + msg.getString("equipmentList"));
                System.out.println("  Office number: " + msg.getString("officeNumber"));
                outstandingRequests.remove(id);
            } catch (JMSException je) {
                logger.log(Level.SEVERE, "HRListener.onMessage(): Exception: {0}", je.toString());
            }
