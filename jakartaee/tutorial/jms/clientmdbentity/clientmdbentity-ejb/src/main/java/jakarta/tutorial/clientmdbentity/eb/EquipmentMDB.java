...
/**
 * The EquipmentMDB class is a message-driven bean. It implements the jakarta.jms.MessageListener
 * interface. It is defined as public (but not final or abstract).
 */
...
@MessageDriven(mappedName = "java:app/jms/HRTopic", activationConfig = {
        ...
        @ActivationConfigProperty(propertyName = "subscriptionName",
                                  propertyValue = "EquipmentMDB")}) public class EquipmentMDB
    ...
    /**
     * onMessage method, declared as public (but not final or static), with a return type of void,
     * and with one argument of type jakarta.jms.Message.
     * <p>
     * Casts the incoming Message to a MapMessage, retrieves its contents, and assigns equipment
     * appropriate to the new hire's position. Calls the compose method to store the information in
     * the persistence entity and, if work is complete, to send a reply message to the client.
     *
        ...
        try {
            if (inMessage instanceof MapMessage) {
                ...
                logger.log(Level.INFO,
                           "EquipmentMDB.onMessage: " + "Message received for employeeId {0}", key);
                switch (position) {
                    case "Programmer":
                ...
                logger.log(Level.WARNING, "EquipmentMDB.onMessage: Message of wrong type: {0}",
                           inMessage.getClass().getName());
            }
        } catch (JMSException | InterruptedException e) {
            logger.log(Level.SEVERE, "EquipmentMDB.onMessage: Exception: {0}", e.toString());
            mdc.setRollbackOnly();
        }
    }
     ...
     * <p>
     * Locates the row of the database represented by the primary key and adds the equipment
     * allocated for the new hire.
     *
    ...
    void compose(String key, String name, String equipmentList, Message msg) {
        ...
        try {
            ...
            if (so != null) {
                logger.log(Level.INFO,
                           "EquipmentMDB.compose: " + "Found join entity for employeeId {0}", key);
            }
        } catch (IllegalArgumentException iae) {
            logger.log(Level.WARNING, "EquipmentMDB.compose: No join entity found: {0}",
                       iae.toString());
        } catch (Exception e) {
            logger.log(Level.SEVERE, "EquipmentMDB.compose: em.find failed without" +
        ...
        if (so == null) {
            try {
                logger.log(Level.INFO,
                           "EquipmentMDB.compose: " + "Creating join entity for employeeId {0}",
                           key);
                so = new SetupOffice(key, name);
                em.persist(so);
            } catch (Exception e) {
                logger.log(Level.WARNING, "EquipmentMDB.compose: Could not create join entity: {0}",
                           e.toString());
                mdc.setRollbackOnly();
            }
        }
        ...
        if (so != null) {
            try {
                ...
                logger.log(Level.SEVERE,
                           "EquipmentMDB.compose: " + "Could not get equipment for employeeId {0}",
                           key);
                mdc.setRollbackOnly();
            }
        }
        ...
        if (done) {
            try {
                ...
                logger.log(Level.INFO,
                           "EquipmentMDB.compose: " + "Sent reply message for employeeId {0}",
                           so.getEmployeeId());
            } catch (JMSException je) {
                logger.log(Level.SEVERE, "EquipmentMDB.compose: " + "JMSException: {0}",
                           je.toString());
            }
    ...
    /**
     * The createReplyMsg method composes the reply message with the new hire information.
     *
