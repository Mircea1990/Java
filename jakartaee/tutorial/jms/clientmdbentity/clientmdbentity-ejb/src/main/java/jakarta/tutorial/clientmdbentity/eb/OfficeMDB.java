...
/**
 * The OfficeMDB class is a message-driven bean. It implements the jakarta.jms.MessageListener
...
public class OfficeMDB implements MessageListener {
     ...
     * <p>
     * Casts the incoming Message to a MapMessage, retrieves its contents, and assigns the new hire
     * to an office. Calls the compose method to store the information in the persistence entity
     * and, if work is complete, to send a reply message to the client.
     *
        ...
        try {
            if (inMessage instanceof MapMessage) {
                ...
                logger.log(Level.INFO,
                           "OfficeMDB.onMessage:" + " Message received for employeeId {0}", key);
                ...
                logger.log(Level.WARNING, "OfficeMDB.onMessage: Message of wrong type: {0}",
                           inMessage.getClass().getName());
            }
        } catch (JMSException | InterruptedException e) {
            logger.log(Level.SEVERE, "OfficeMDB.onMessage: Exception: {0}", e.toString());
            mdc.setRollbackOnly();
        }
    }
     ...
     * <p>
     * Locates the row of the database represented by the primary key and adds the office number
    ...
    void compose(String key, String name, int officeNumber, Message msg) {
        ...
        try {
            ...
            if (so != null) {
                logger.log(Level.INFO,
                           "OfficeMDB.compose: " + "Found join entity for employeeId {0}", key);
            }
        ...
        if (so == null) {
            try {
                logger.log(Level.INFO,
                           "OfficeMDB.compose: " + "Creating join entity for employeeId {0}", key);
        ...
        if (so != null) {
            try {
                ...
                logger.log(Level.SEVERE,
                           "OfficeMDB.compose: " + "Could not get office for employeeId {0}", key);
                mdc.setRollbackOnly();
            }
        }
        ...
        if (done) {
            try {
                ...
                logger.log(Level.INFO,
                           "OfficeMDB.compose: " + "Sent reply message for employeeId {0}",
                           so.getEmployeeId());
            } catch (JMSException je) {
                logger.log(Level.SEVERE, "OfficeMDB.compose: " + "JMSException: {0}",
                           je.toString());
            }
