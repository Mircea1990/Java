...
@Entity public class SetupOffice implements Serializable {
    ...
    /**
     * The doEquipmentList method stores the assigned equipment in the database, then determines if
     * setup is complete.
     *
    ...
    public boolean doEquipmentList(String list) {
        ...
        logger.log(Level.INFO, "SetupOffice.doEquipmentList: Equipment for " +
                   ...
                   new Object[]{getEmployeeId(), getEquipmentList(), getOfficeNumber()});
        done = checkIfSetupComplete();
        return done;
    }

    /**
     * The doOfficeNumber method stores the assigned office number in the database, then determines
    ...
    public boolean doOfficeNumber(int officeNum) {
        ...
        logger.log(Level.INFO, "SetupOffice.doOfficeNumber: Office number for " +
                   ...
                   new Object[]{getEmployeeId(), getOfficeNumber(), getEquipmentList()});
        done = checkIfSetupComplete();
        return done;
    }

    /**
     * The checkIfSetupComplete method determines whether both the office and the equipment have
     * been assigned. If so, it reports that the work of the entity is done.
     *
     * @return true if setup is complete
     */
    private boolean checkIfSetupComplete() {
        ...
        if ((getEquipmentList() != null) && (getOfficeNumber() != -1)) {
            logger.log(Level.INFO, "SetupOffice.checkIfSetupComplete:" +
                                   ...
                                   " office #{2} with {3}",
                       new Object[]{getEmployeeId(), getEmployeeName(), getOfficeNumber(),
                               getEquipmentList()});
