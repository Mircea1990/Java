...
public class GenericSupplier {
    ...
    @Resource(lookup = "java:comp/DefaultJMSConnectionFactory") private static ConnectionFactory
            connectionFactory;
    @Resource(lookup = "jms/OTopic") private static Topic supplierOrderTopic;

    /**
     * Constructor. Instantiates the supplier as the supplier for the kind of item to be ordered.
     *
            ...
            try (JMSContext context = connectionFactory.createContext(
                ...
                while (true) {
                    try {
                        ...
                        System.out.println(
                                PRODUCT_NAME + " Supplier: Vendor ordered " + quantity + " " +
                                PRODUCT_NAME + "(s)");
                        ...
                        context.createProducer().send((Queue) orderMessage.getJMSReplyTo(),
                                                      outMessage);
                        System.out.println(
                                PRODUCT_NAME + " Supplier: Sent " + outMessage.getInt("Quantity") +
                                " " + outMessage.getString("Item") + "(s)");
                        context.commit();
                        System.out.println(
                                "  " + PRODUCT_NAME + " Supplier: Committed transaction");
                    } catch (Exception e) {
                        System.err.println(
                                PRODUCT_NAME + " Supplier: Exception occurred: " + e.toString());
                    }
                }
            } catch (JMSRuntimeException ee) {
                System.err.println(
                        PRODUCT_NAME + " Supplier: Exception occurred: " + ee.toString());
            }
        }
    }

    /**
     * Timer method. Completes when ready is set to true, after context is started. Sleep prevents
     * supplier from getting ahead of itself on fast machines.
     */
