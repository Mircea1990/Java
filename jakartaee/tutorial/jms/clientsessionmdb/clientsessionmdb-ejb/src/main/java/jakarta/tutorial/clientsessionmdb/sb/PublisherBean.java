...
/**
 * Bean class for Publisher enterprise bean. Defines publishNews business method as well as required
 * methods for a stateless session bean.
 */
@Stateless @Remote({PublisherRemote.class}) public class PublisherBean implements PublisherRemote {
    ...
    /**
     * Chooses a message type by using the random number generator found in java.util. Called by
     * publishNews().
     *
    ...
    /**
     * Creates producer and message. Sends messages after setting their NewsType property and using
     * the property value as the message text. Messages are received by MessageBean, a
     * message-driven bean that uses a message selector to retrieve messages whose NewsType property
     * has certain values.
     */
    @Override public void publishNews() {
        TextMessage message;
        ...
        try {
            ...
            for (int i = 0; i < numMsgs; i++) {
                ...
                logger.log(Level.INFO, "PUBLISHER: Setting message text to: {0}",
                           message.getText());
                context.createProducer().send(topic, message);
            }
        } catch (JMSException t) {
            logger.log(Level.SEVERE, "PublisherBean.publishNews: Exception: {0}", t.toString());
            sc.setRollbackOnly();
        }
    }
}
