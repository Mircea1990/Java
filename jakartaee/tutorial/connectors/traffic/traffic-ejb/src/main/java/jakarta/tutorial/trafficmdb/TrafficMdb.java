...
@JMSDestinationDefinition(name = "java:app/traffic-ejb/traffictopic",
                          interfaceName = "jakarta.jms.Topic", destinationName = "CityTopic")
/* Activate the traffic-rar resource adapter from this MDB */ @MessageDriven(activationConfig = {
        @ActivationConfigProperty(propertyName = "port", propertyValue = "4008")})
/* Receive messages from the EIS and filter them for "City1" only */ public class TrafficMdb
    ...
    @Resource(lookup = "java:app/traffic-ejb/traffictopic") private Topic topic;
    ...
    public void processReport(String jsonReport) {
        ...
        if (parser.hasNext() && parser.next() == JsonParser.Event.START_OBJECT &&
            ...
            parser.getString().equals("report") && parser.hasNext() &&
            parser.next() == JsonParser.Event.START_ARRAY && parser.hasNext()) {
