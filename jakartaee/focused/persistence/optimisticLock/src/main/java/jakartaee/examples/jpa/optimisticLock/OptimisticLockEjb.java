...
@Stateless public class OptimisticLockEjb {

    /**
     * Stores the persistence context.
     */
    @PersistenceContext(unitName = "optimisticLockPU") private EntityManager em;
    ...
    public Department findDepartmentByName(String name) {
        return (Department) em.createNamedQuery("Department.findByName").setParameter("name", name)
                              .getSingleResult();
    }
    ...
    public Employee findEmployeeByCode(String code, String departmentName) {
        return (Employee) em.createNamedQuery("Employee.findByCode").setParameter("code", code)
    ...
    public Long findCountOfUniformsInADepartmentWithNoLock(String departmentName) {
        Query query = em.createNamedQuery("Department.findCountOfUniforms").setParameter("name",
                                                                                         departmentName);
     ...
     */
    public void giveUniforms(String employeeCode, Integer count, String departmentName)
    ...
    public Long findUniformCountByCode(String employeeCode) {
        return (Long) em.createNamedQuery("Employee.findUniformCountByCode").setParameter("code",
                                                                                          employeeCode)
                        .setParameter("departmentName", "_X").getSingleResult();
    }

    /**
     * Find total number of uniforms have been given to employees.
     *
     * @param departmentName Name of department for which finding total number of assigned
     *                       uniforms.
     * @return Total number of uniforms have been given to employees of department
     * @throws RuntimeException
     */
    public Long findCountOfUniformsInADepartmentWithOptimisticLock(String departmentName)
            throws RuntimeException {
        try {
            Query query = em.createNamedQuery("Department.findByName").setParameter("name",
                                                                                    departmentName.trim())
            ...
            for (Employee emp : department.getEmployees()) {
                ...
                System.out.println(
                        "Locking employee #" + emp.getCode() + " with " + emp.getUniforms().size() +
                        " uniforms");
    ...
    public Long findDepartmentCount() {
        return (Long) em.createNamedQuery("Department.findDepartmentCount").getSingleResult();
    }

    public Long findEmployeeCount() {
        return (Long) em.createNamedQuery("Employee.findEmployeeCount").getSingleResult();
    }

    public Long findUniformCount() {
        return (Long) em.createNamedQuery("Uniform.findUniformCount").getSingleResult();
    }
